...
@section('content')
    ...
    <form action="{{ route('seller.product.update', ['product' => $product->id]) }}" enctype="multipart/form-data"
        method="POST">
        @method('PUT')
        @csrf
                    ...
                    @if ($product->images->isNotEmpty())
                        @foreach ($product->images as $image)
                            <div class="d-flex flex-column justify-content-center m-2">
                                <img class="m-2" src="{{ asset('images/seller_images/' . $image->image) }}"
    ...
    @foreach ($product->images as $image)
        <div class=" d-inline">
            <img class="m-2" src="{{ asset('images/seller_images/' . $image->image) }}" width="100px" height="100px"
                alt="Product Image">
            <form action="{{ route('seller.destroy.image', ['product' => $product->id, 'image' => $image->id]) }}"
